        ...
        if log:
            if self.hparams["modules"][0] == "ddpg":
                if self.hparams["buffer"][0] == "replay":
                    self.writer.add_scalar('layer_0/replay_buffer size', self.layers[0].replay_buffer.get_current_size(), step)
                elif self.hparams["buffer"][0] == "transitions":  
                    self.writer.add_scalar('layer_0/transitions_buffer size', self.layers[0].transitions_buffer.get_current_size(), step)
                    ...
                    self.writer.add_scalar('layer_0/experience_buffer size', self.layers[0].experience_buffer.size, step)
                ...
                self.writer.add_histogram('layer_0/actor_weights_main', self.layers[0].policy._vars('main/pi/_0')[0].eval(session=self.sess), step)
                self.writer.add_histogram('layer_0/critic_weights_main', self.layers[0].policy._vars('main/Q/_0')[0].eval(session=self.sess), step)
                self.writer.add_histogram('layer_0/actor_weights_target', self.layers[0].policy._vars('target/pi/_0')[0].eval(session=self.sess), step)
                self.writer.add_histogram('layer_0/critic_weights_target', self.layers[0].policy._vars('target/Q/_0')[0].eval(session=self.sess), step)

                ret.append(self.layers[0].policy.critic_loss)

            if self.hparams["layers"] > 1:
                if self.hparams["buffer"][1] == "replay":
                    self.writer.add_scalar('layer_1/replay_buffer size', self.layers[1].replay_buffer.get_current_size(), step)
                elif self.hparams["buffer"][1] == "transitions":  
                    self.writer.add_scalar('layer_1/transitions_buffer size', self.layers[1].transitions_buffer.get_current_size(), step)
                elif self.hparams["buffer"][1] == "experience":  
                    self.writer.add_scalar('layer_1/experience_buffer size', self.layers[1].experience_buffer.size, step)


                if self.hparams["modules"][1] == "actorcritic":
                    # Logging exemplary weight to verify transfer learning
                    self.writer.add_histogram('layer_1/critic_weights_main', self.layers[1].critic.weights[0].eval(session=self.sess), step)
